    ...
    <div class="container">
        <div class="w-full p-2">
            ...
            <div class="card card-body shadow">
                {{-- search by bulan, tanggal dan status --}}
                <form action="{{ url('report/peminjaman') }}" method="get">
                    <div class="flex justify-between w-full">
                        <h2 class="inline no-wrap">
                            Laporan Peminjaman Alat Musik
                        </h2>
                        <div class="flex justify-end items-center  mb-5">
                            <div class="form-group mr-2">
                                {{-- input type month --}}
                                <input class="form-control" id="bulan" name="bulan" type="month"
                                    value="{{ request()->get('bulan') ?? '' }}">
                            </div>
                            ...
                            <div class="form-group mr-2">
                                <select class="form-control" id="status" name="status">
                                    ...
                                    <option
                                        {{ !empty(request()->get('status')) && request()->get('status') == 'pending' ? 'selected' : '' }}
                                        value="pending">Menunggu Tanggapan</option>
                                    <option
                                        {{ !empty(request()->get('status')) && request()->get('status') == 'approved' ? 'selected' : '' }}
                                        value="approved">Disetujui</option>
                                    <option
                                        {{ !empty(request()->get('status')) && request()->get('status') == 'borrowed' ? 'selected' : '' }}
                                        value="borrowed">Sedang Dipinjam</option>
                                    <option
                                        {{ !empty(request()->get('status')) && request()->get('status') == 'rejected' ? 'selected' : '' }}
                                        value="rejected">Ditolak</option>
                                    <option
                                        {{ !empty(request()->get('status')) && request()->get('status') == 'returned' ? 'selected' : '' }}
                                        value="returned">Telah Dikembalikan</option>
                                </select>
                            </div>
                            <div class="form-group mr-2">
                            ...
                            <div class="form-group mr-2">
                                <a class="btn btn-secondary" href="#" id="print">
...
@push('script')
    ...
    <script>
        ...
        $("#print").click(function() {
            ...
            // Tentukan URL tujuan
            var baseUrl = "{{ url('report/print-laporan') }}";

            // Jika ada query parameter, tambahkan ke URL tujuan
            if (queryParams) {
                baseUrl += '?' + queryParams;
            }
